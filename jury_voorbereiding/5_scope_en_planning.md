...
- **Authenticatie & Autorisatie:** Standaard login/registratie én Social Login via GitHub. Rollen-systeem (Admin vs Klant).
...
- *Uitgebreid Adresboek / Klantprofiel:* Bij het afrekenen moet het adres handmatig worden ingevuld. Dit is een bewuste keuze geweest om de focus te houden op een waterdichte Stripe-koppeling en de kern van de webshop in plaats van 'nice-to-have' profiel-features.
- *Automatisch Voorraadbeheer via Webhooks:* De voorraad (`stock`) wordt momenteel wel bijgehouden per variant, maar wordt nog niet automatisch verlaagd na een succesvolle betaling.
...
Tijdens de ontwikkeling is gewerkt met duidelijke mijlpalen (Fases), zoals terug te vinden in de `progress.md`:
1. **Fase 1-3:** Basis setup, Database ontwerp, Auth & Social Login.
